...
// DO NOT EDIT FUNCTION// we edited the function and added helloText.style.display = 'block'
...
//document.getElementById('repeat-button').addEventListener('click',() => {
    ...
    // axios.get('http://localhost:3000/repeat/pizza-is-love')
    // .then((res) =>{
...
function sendAQuery(event) {
    axios.get('http://localhost:3000/query-test?q=${query.value}')
    .then((res) => {
        console.log(res.data)
    })
}
...
const ohMy = () => {
    ...
    .then((res) => {
        for (let i=0; i < res.data.length; i++){
            let newString = document.createElement('p').textContent=res.data[i]
        }
    })
    }
